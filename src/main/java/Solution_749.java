...
public class Solution_749 {
    ...
    private void getPosOne(int[][] grid, int x, int y, int[][] serial, int flag, HashSet<String> access) {
        if (x >= grid.length || y >= grid[0].length || x < 0 || y < 0 || grid[x][y] == 0 || grid[x][y] == -2) return;
        String s = x + "_" + y;
        if (access.contains(s)) return;
        access.add(s);
        grid[x][y] = -2;
        serial[x][y] = flag;
    ...
    private void infection(int[][] grid, List<Info> areaOneNum, Set<String> set) {
        ...
        IntStream.range(0, list.size()).forEach(i -> grid[list.get(i).x][list.get(i).y] = 1);

    }

    private void infeArea(int[][] grid, int preX, int preY, int x, int y, Set<String> set, List<Info> list, HashSet<String> access) {
        if (x >= grid.length || y >= grid[0].length || x < 0 || y < 0 || grid[x][y] == -1 || grid[x][y] == -2) return;
        if (grid[x][y] == 0) {
            String vireCode = getCode(preX, preY, x, y);
            if (!set.contains(vireCode)) list.add(new Info(x, y));
            return;
        }
