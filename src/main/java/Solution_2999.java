...
public class Solution_2999 {
    ...
    private long dfs(int i, boolean limit_low, boolean limit_high,
        ...
        long res = 0;
        if (i < pre_len)
            res = IntStream.rangeClosed(lo, Math.min(hi, limit)).mapToLong(digit -> dfs(i + 1, limit_low && digit == lo,
                    limit_high && digit == hi,
                    low, high, limit, s, pre_len, memo)).sum();
        else {
            int x = s.charAt(i - pre_len) - '0';
            if (lo <= x && x <= Math.min(hi, limit)) res = dfs(i + 1, limit_low && x == lo,
                    limit_high && x == hi,
